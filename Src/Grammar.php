...
class Grammar
{
    ...
    public function arrayConnectDuplicate(
        ...
        if ($replaceIt) {
            $output = array_map(function ($val) use ($replaceIt) {
                return $replaceIt;
            }, $array);
        } else {
            $output = $array;
    ...
    // chang syntax  
    public function arrayBetweenSub(
        array $values,
        string $between_before = '`',
        string $between_after = '`',
        string $subStr = ' , ',
        $replaceIt = null,
        $before = false
    ) {
    ...
    // append string for all value in araay or compper an before
    public function appendStrArray(array &$array, string $before, string $after = '', $values = null)
    {
        if ($values) {
            foreach ($values as $v) {
                $key = array_search($v,$array,true);
                if($key !== false){
                    $array[$key] = $before . $v . $after;
                }
            }
        } else {
            foreach ($array as $k => $v) {
                $array[$k] = $before . $v . $after;
            }
        }
    }
}
